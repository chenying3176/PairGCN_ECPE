...
class CausePairLoader():
	...
	def generate_loader(self, data, batch_size, shuffle, drop_last):
		...
		tensor_batch = self._get_tensor(all_batch)
		return tensor_batch
	...
	def generate_fold_data(self, fold_num, batch_size, shuffle, drop_last):
		...
		if not os.path.exists(tmp_data_dir) or shuffle:
			...
			dev_loader = self.generate_loader(dev_data, batch_size, shuffle, drop_last)
			test_loader = self.generate_loader(test_data, batch_size, shuffle, drop_last)
		else:
			train_loader = torch.load(tmp_data_dir+'.train')
			test_loader = torch.load(tmp_data_dir+'.test')
		return train_loader, dev_loader, test_loader

	def generate_loader2(self, data, batch_size, sample_index):
		def generate_batch(data, sample_size, batch_size):
			return [data[i:i+batch_size] for i in range(0, sample_size, batch_size)]
		ids, clens, slens, labels, emotions, events = data
		sample_size = len(ids)
		...
		ids = [ids[index] for index in sort_idx]
		labels = [labels[index] for index in sort_idx]
		sample_index = [sample_index[index] for index in sort_idx]

		ids_batch = generate_batch(ids, sample_size, batch_size)
		labels_batch = generate_batch(labels, sample_size, batch_size)
		sample_index = generate_batch(sample_index, sample_size, batch_size)
		tensor_batch = (ids_batch, labels_batch)
		return tensor_batch, sample_index

	def generate_fold_data2(self, fold_num, batch_size, shuffle, drop_last):
		tmp_data_dir = os.path.join(self.tmp_dir, 'tmp_data_%d'%fold_num)

		if not os.path.exists(tmp_data_dir) or shuffle:
			cur_fold_index = self.raw_data.fold_index[fold_num]
			train_index, test_index = cur_fold_index
			train_index = [index for index in train_index if index in self.raw_data.data]
			test_index = [index for index in test_index if index in self.raw_data.data]

			train_data, test_data = \
			[self.raw_data.data[index] for index in train_index], \
			[self.raw_data.data[index] for index in test_index]

			train_ids, train_clen, train_slen, train_labels, train_emotions, train_events\
				 = self._convert_sample2id(train_data)
			test_ids, test_clen, test_slen, test_labels, test_emotions, test_events\
			 = self._convert_sample2id(test_data)
			train_data = (train_ids, train_clen, train_slen, train_labels, train_emotions, train_events)
			test_data = (test_ids, test_clen, test_slen, test_labels, test_emotions, test_events)

			train_loader, self.train_index = self.generate_loader2(train_data, batch_size, train_index)
			test_loader, self.test_index = self.generate_loader2(test_data, batch_size, test_index)
	...
	def _get_tensor(self, batch):
		...
		for ids, clen, slen, labels, emotions, events in zip(*batch):
			...
			for cur_idx, cids, cl, sl, label, emotion, event \
				...
				bfs = [self.raw_data.bert_feat[cid] for cid in cids]
				cl = [bf.size(0) for bf in bfs]
				cids_batch.extend(bfs)
			...
			# cids_tensor = cids_batch
			cids_tensor = pad_sequence(cids_batch, batch_first=True).to(self.device)
	...
	def _convert_sample2id(self, samples):
		...
		for sam in samples:
			...
			for clause, emo in zip(sam[0], sam[2]):
				clause_ids = ''.join([word for word in clause.split()][:self.max_sen_len]).replace(' ', '')
...
class CausePairRawData():
	def __init__(self, inp_files):
		...
		self.bert_feat = self.load_bert_feat()
	...
	def load_bert_feat(self):
		bert_feat = {}
		bert_feat_files = os.listdir('data/')
		for bff in bert_feat_files:
			if 'all_bert_feat' not in bff:
				continue
			bf = torch.load('data/'+bff)
			for key in bf:
				bert_feat[key] = bf[key]
		return bert_feat
